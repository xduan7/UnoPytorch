...
class RespNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.__resp_net.add_module(
            'dense_%d' % 0,  nn.Linear(gene_latent_dim + drug_latent_dim + 1,
                                       resp_layer_dim))
        ...
        for i in range(resp_num_blocks):
            self.__resp_net.add_module(
                ...
                ResBlock(layer_dim=resp_layer_dim,
                         num_layers=resp_num_layers,
                         dropout=resp_dropout))
        self.__resp_net.add_module('dense_%d' % resp_num_blocks,
                                   nn.Linear(resp_layer_dim, 1))
